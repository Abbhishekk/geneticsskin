  ...
  function convert_filesize($bytes, $decimals = 2) { 
    ...
    if(isset($_POST["submit"])){
        ...
        if(isset($_FILES["before"]["name"]) &&isset($_FILES["after"]["name"]) && !empty($_FILES["before"]["name"]) && !empty($_FILES["after"]["name"])){
            ...
            $imageSize = convert_filesize($_FILES["before"]["size"]); 
             
            // Compress size and upload image 
            $compressedImage_before = compressImage($imageTemp_before, $file_name_before, 25); 

            $imageTemp_after = $_FILES["after"]["tmp_name"]; 
            $imageSize = convert_filesize($_FILES["after"]["size"]); 
             
            // Compress size and upload image 
            $compressedImage = compressImage($imageTemp_after, $file_name_after, 25); 
            if($compressedImage_before && $compressedImage){
